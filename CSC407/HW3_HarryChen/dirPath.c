 ...
 *--------------------------------------*
 *------*
 *---Version 1.02018 February 23Joseph Phillips---*
 *------*
 *-------------------------------------------------------------------------*/
		...
		)
{
  ...
  //  YOUR CODE HERE
  for (; isLegalDirEntryChar(*linePtr); linePtr++) {  }
  ...
  //  YOUR CODE HERE
  //specify the size of name variable in heap
  int entryLen = linePtr - entryNamePtr;
  struct DirEntryName* entry = (struct DirEntryName*)malloc(sizeof(struct DirEntryName));
  char* name=(char*)malloc(sizeof(char) * (entryLen + 1));
  //copy entryNamePtr which has lens entryLen to variable name
  memcpy(name, entryNamePtr, entryLen);
  name[entryLen] = '\0';
  //char* name = niceCopy(entryNamePtr);
  entry->name_ = name;
  ...
  //       'nextPtr_' to 'NULL'.
  //  YOUR CODE HERE
  if (*linePtr == '/') entry->nextPtr_ = parseRestOfPath(linePtr + 1);
  if (*linePtr == '\0') entry->nextPtr_ = NULL;
  //  III.  Finished:
